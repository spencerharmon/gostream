...
import (
	...
	// Pure-Go, in-process SQL engine used ONLY to exercise the DB code path offline (no
	// external server, no CGO). Production uses the pgx/PostgreSQL driver; the SQL is
	// kept portable so the identical Get/Set/List/Rem/Clear logic runs here.
	_ "modernc.org/sqlite"
)

// kv is the minimal Get/Set/List/Rem/Clear/CloseDB contract that XPathDBRouter relies
// on. *DB satisfies it; declaring it here keeps the contract test backend-agnostic
// without importing the CGO-heavy settings package.
type kv interface {
	CloseDB()
...
// contract exercises the full KV contract that XPathDBRouter relies on. It is
// backend-agnostic: run against the in-process engine always, and against a real
// PostgreSQL server when GOSTORM_PG_TEST_DSN is provided.
func contract(t *testing.T, db kv) {
	t.Helper()

	// Set/Get round-trip (bytes preserved exactly, including NUL and high bytes).
...
// TestPostgresBackend verifies the Postgres KV backend end-to-end.
//
// The "inprocess" subtest runs the identical DB code path against an embedded pure-Go
// SQL engine, so the KV/bucket contract is genuinely verified in the offline check
// sandbox (it fails to compile/pass without postgres.go and pulls NO torrent CGO deps).
// When GOSTORM_PG_TEST_DSN names a reachable PostgreSQL logical database (e.g.
// gostream_dev on the shared server), the "postgres" subtest runs the SAME contract
// against the real pgx driver.
func TestPostgresBackend(t *testing.T) {
	t.Run("inprocess", func(t *testing.T) {
		dsn := filepath.Join(t.TempDir(), "gostorm_settings_test.db")
		db, err := newWithDriver("sqlite", dsn)
		if err != nil {
