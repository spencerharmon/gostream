...
type XPathDBRouter struct {
	dbs      []GoStormDB
	routes   []string
	route2db map[string]GoStormDB
	dbNames  map[GoStormDB]string
}
...
func (v *XPathDBRouter) RegisterRoute(db GoStormDB, xPath string) error {
...
func (v *XPathDBRouter) getDBForXPath(xPath string) GoStormDB {
	if len(v.dbs) == 0 {
		return nil
	}
	lookup_route := v.xPathToRoute(xPath)
	var db GoStormDB = nil
...
func (v *XPathDBRouter) getDBName(db GoStormDB) string {
	return v.dbNames[db]
}
