...
func (cn *PeerConn) write(msg pp.Message) bool {
	// V255: Removed duplicate torrent.Add(fmt.Sprintf(...)) - wroteMsg() already counts this.
...
func (cn *PeerConn) choke(msg messageWriter) (more bool) {
	...
	if !cn.fastEnabled() {
		// HACKER MODE V234: Don't flush requests on choke.
		// This keeps "pressure" on the peer so when they unchoke, we are ready instantly.
...
func (cn *PeerConn) have(piece pieceIndex) {
	...
	// HACKER MODE V234: Ghost Protocol DISABLED (Back to Standard Visibility)
	cn.write(pp.Message{
		Type:  pp.Have,
...
func (cn *PeerConn) wroteMsg(msg *pp.Message) {
	// V255: Use string concatenation instead of fmt.Sprintf to avoid allocation per message.
	torrent.Add("messages written of type "+msg.Type.String(), 1)
	if msg.Type == pp.Extended {
		for name, id := range cn.PeerExtensionIDs {
			if id != msg.ExtendedID {
				continue
			}
			torrent.Add("Extended messages written for protocol "+string(name), 1)
		}
	}
