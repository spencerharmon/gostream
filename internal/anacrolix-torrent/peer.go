...
func (cn *Peer) nominalMaxRequests() maxRequests {
	...
	if expectingTime == 0 {
		// V255: Initial pipeline 16 instead of 2 to saturate the pipe on first RTT.
		// 25 peers × 16 reqs × 16KB = 6.4MB in flight immediately → faster TTFF.
		return 16
	}
	...
	// Sanity cap to avoid infinite memory growth on massive pipes
	if pipeline > 150 {
		pipeline = 150
	}

	// 4. Respect Peer's Limit (Crucial for anti-ban)
	limit := int64(cn.PeerMaxRequests)
	if limit > 0 && pipeline > limit {
		pipeline = limit
	}

	return maxRequests(pipeline)
}
...
func (c *Peer) receiveChunk(msg *pp.Message) error {
	...
	if t.pieceAllDirty(pieceIndex(ppReq.Index)) && piece.atomicPendingWrites.Load() == 0 {
